...
metadata_SHD = pd.read_csv('data/ShanghaiDogsMetadata/SH_Dog_metadata_red.csv', \
                       sep=',', encoding= 'unicode_escape', index_col=0)
...
tax_w_metadata = pd.merge(tax_profile_filt,
                          metadata_filt[['env_classification','Animal_age_simplified','Body_condition','Sex','Study','Size_class']],
                          left_index=True,right_index=True)

### Check KW diff between groups, as an alternative approach to Maaslin2
variable_ls = ['env_classification', 'Animal_age_simplified', 'Body_condition', 'Sex', 'Study', 'Size_class']
kruskal_all = []
...
def filter_data(df, variable):
    ...
    if variable in ['Animal_age_simplified', 'Body_condition', 'Sex', 'Size_class']:
...
for variable in variable_ls:
    ...
    tax_w_metadata = pd.merge(tax_profile_filt, metadata_filt[
        ['env_classification', 'Animal_age_simplified', 'Body_condition', 'Sex', 'Study', 'Size_class']],
                              left_index=True, right_index=True)
...
### SHANGHAI DOGS KW test

### Filter tax-profile to include just the relevant samples
tax_profile_filt = tax_profile[tax_profile.columns[tax_profile.columns.isin(metadata_SHD.index)]]
tax_profile_filt = tax_profile_filt.T
tax_w_metadata_SH = pd.merge(tax_profile_filt, metadata_SHD,
                          left_index=True,right_index=True)

### Check KW diff between groups, as an alternative approach to Maaslin2
variable_ls = ['Supplement_frequency', 'Other_cats', 'Time_no_humans']
kruskal_SHD = []
...
for variable in variable_ls:
    ...
    # Filter the data for the variable
    tax_metadata_filt = filter_data(tax_w_metadata_SH, variable)
    print(tax_metadata_filt[variable].value_counts())
    ...
    for tax in tax_metadata_filt.columns:
        ...
        if len(grouped_data) > 1:
            ...
            if len(non_zero_variance_groups) > 1:  # Proceed only if at least two groups have non-zero variance
                ...
                # Store the results (variable, taxon, statistic, p-value)
                kruskal_SHD.append((variable, tax, kw_stat, kw_pval))

# Convert the results to a DataFrame for easy interpretation
kruskal_SHD_df = pd.DataFrame(kruskal_SHD, columns=['Variable', 'Taxonomy', 'KW_Statistic', 'p_value'])

# Bonferroni correction & Benjamini-Hochberg FDR correction
kruskal_SHD_df['p_value_bonferroni'] = multipletests(kruskal_SHD_df['p_value'], method='bonferroni')[1]
kruskal_SHD_df['p_value_fdr'] = multipletests(kruskal_SHD_df['p_value'], method='fdr_bh')[1]

# Print or save the results
print(kruskal_SHD_df.head())
